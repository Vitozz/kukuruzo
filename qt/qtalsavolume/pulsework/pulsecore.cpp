...
PulseCore::PulseCore(const char *clientName) :
    mainLoop_(pa_mainloop_new()), mainLoopApi_(pa_mainloop_get_api(mainLoop_)),
    context_(pa_context_new(mainLoopApi_, clientName)), retval_(0), currentDeviceName_(QString()),
    currentDeviceIndex_(0), sinksDescriptions_(QStringList()), sourcesDescriptions_(QStringList()),
...
QString PulseCore::getDeviceNameByIndex(int index) { return getDeviceByIndex(index)->name(); }
...
void PulseCore::onError(const QString &message) { QMessageBox::critical(nullptr, "Error in pulsecore.cpp", message); }
...
void PulseCore::setVolume(int value) { setVolume_(getDeviceByIndex(currentDeviceIndex_), value); }

void PulseCore::setMute(bool mute) { setMute_(getDeviceByIndex(currentDeviceIndex_), mute); }
...
int PulseCore::getVolume() { return getDeviceByIndex(currentDeviceIndex_)->volume_percent(); }

int PulseCore::getCardIndex() { return getDeviceByIndex(currentDeviceIndex_)->card(); }

bool PulseCore::getMute() { return getDeviceByIndex(currentDeviceIndex_)->mute(); }

const QStringList &PulseCore::getCardList() const { return deviceDescriptions_; }

PulseDevice::Ptr PulseCore::getDeviceByIndex(int index)
{
    updateDevices();
    const int        deltaIndex = sinksDescriptions_.size() - index;
    PulseDevice::Ptr device     = (deltaIndex > 0) ? getSink(index) : getSource(abs(deltaIndex));
    return device;
}

int PulseCore::getCurrentDeviceIndex() const { return deviceNames_.indexOf(currentDeviceName_); }
