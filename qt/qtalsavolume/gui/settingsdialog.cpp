...
SettingsDialog::SettingsDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::SettingsDialog), soundCards_(QStringList()),
      mixers_(QStringList()), isAutorun_(false), pulseAvailable_(false),
      playbacks_(new QListWidget(this)), captures_(new QListWidget(this)),
...
void SettingsDialog::connectSignals() {
  connect(playbacks_, &QListWidget::itemChanged, this,
          &SettingsDialog::onPBAction);
  connect(captures_, &QListWidget::itemChanged, this,
          &SettingsDialog::onCPAction);
  connect(enums_, &QListWidget::itemChanged, this, &SettingsDialog::onENAction);
  connect(ui->darkRadio, &QRadioButton::toggled, this,
          &SettingsDialog::onDarkStyle);
  connect(ui->lightRadio, &QRadioButton::toggled, this,
          &SettingsDialog::onLightStyle);
  connect(ui->cardBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this](int index) { emit soundCardChanged(index); });
  connect(ui->mixerBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this](int index) {
            if (index <= ui->mixerBox->count()) {
              emit mixerChanged(ui->mixerBox->itemText(index));
            }
          });
  connect(ui->isAutorun, &QCheckBox::toggled, this,
          [this](bool toggle) { emit autorunChanged(toggle); });
  connect(ui->usePulseaudio, &QCheckBox::toggled, this,
          &SettingsDialog::onPulseSoundSystem);
  connect(ui->enableTimer, &QCheckBox::toggled, this,
          &SettingsDialog::onEnableTimer);
}
...
void SettingsDialog::closeEvent(QCloseEvent *) { hide(); }
...
void SettingsDialog::onEnableTimer(bool toggled) { emit timerEnabled(toggled); }
