...
langs_url="https://github.com/psi-im/psi-l10n.git"
...
fetch_all ()
{
  if [ $isoffline -eq 0 ]; then
    ...
    fetch_url ${langs_url} ${buildpsi}/psi-langs
  fi
}
...
get_psi_version()
{
  local plus_tag=$(cd ${upstream_src} && git describe --tags | cut -d - -f1)
  local psi_num=$("${upstream_src}/admin/git_revnumber.sh" "${plus_tag}")
  psi_version="${plus_tag}"
  psi_package_version="${plus_tag}.${psi_num}"
  echo "SHORT_VERSION = $psi_package_version"
  echo "LONG_VERSION = $psi_package_version"
}
...
prepare_workspace ()
{
  ...
  cp -a ${buildpsi}/psi-langs/translations/*.ts ${workdir}/translations/
  cd ${workdir}
  get_psi_version
  local suffix=""
  ...
  echo $psi_version > ${workdir}/version
}
...
compile_psiplus ()
{
  ...
  cd ${builddir}
  flags="-DCMAKE_BUILD_TYPE=${DEF_CMAKE_BUILD_TYPE} -DPSI_LIBDIR=${buildpsi}/build-plugins -DPRODUCTION=OFF"
  if [ ! -z "$1" ]; then
...
install_locales ()
{
  local tr_path=${buildpsi}/psi-langs/translations
  build_locales
  check_dir ${psi_datadir}
...
compile_psi_mxe()
{
  ...
  flags="${flags} -DUSE_CCACHE=ON -DVERBOSE_PROGRAM_NAME=ON -DPRODUCTION=OFF"
  wrkdir=${builddir}
  check_dir ${wrkdir}
  cd ${wrkdir}
  echo "--Starting cmake
...
archivate_all()
{
  wbk_suff="all-"
  mxe_outd=${tmp_dir}/mxe_builds
  7z a -mx=9 -m0=LZMA -mmt=on ${mxe_outd}/psi-${wbk_suff}${psi_package_version}-$1.7z ${mxe_outd}/$1/*
  cp -r ${mxe_outd}/psi-${wbk_suff}${psi_package_version}-$1.7z ${buildpsi}/mxe_builds/
}
#Запись настроек
set_config ()
{
...
---${pink}[31]${nocolor} - Build psi plugins
${pink}[6]${nocolor} - Set options
${pink}[8]${nocolor} - Get help on additional actions
${pink}[9]${nocolor} - Run compiled psi binary
${pink}[0]${nocolor} - Exit"
}
#Справка по командам
get_help ()
