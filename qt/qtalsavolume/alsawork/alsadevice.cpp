...
AlsaDevice::AlsaDevice(int id, QString card) :
    id_(id), name_(std::move(card)), volumeMixers_(QStringList()), captureMixers_(QStringList()),
    mixers_(QStringList()), switches_(MixerSwitches::Ptr(new MixerSwitches())), currentMixerId_(0),
...
void AlsaDevice::updateElements()
{
    ...
    auto                  handle = getMixerHanlde(id_);
...
void AlsaDevice::setDeviceVolume(double volume)
{
    if (!currentMixerName_.isEmpty() && !mixers_.isEmpty()) {
        auto              handle  = getMixerHanlde(id_);
        snd_mixer_elem_t *element = initMixerElement(handle.get(), currentMixerName_.toLocal8Bit());
        if (!snd_mixer_elem_empty(element)) {
...
double AlsaDevice::getVolume()
{
    ...
    if (!currentMixerName_.isEmpty() && !mixers_.isEmpty()) {
        auto              handle = getMixerHanlde(id_);
        snd_mixer_elem_t *elem   = initMixerElement(handle.get(), currentMixerName_.toLocal8Bit());
...
{
    auto              handle = getMixerHanlde(id_);
    snd_mixer_elem_t *elem   = initMixerElement(handle.get(), mixer.toLocal8Bit());
    if (!snd_mixer_elem_empty(elem)) {
        switch (id) {
        case PLAYBACK:
        default:
...
QString AlsaDevice::formatCardName(int id) { return QString("hw:%1").arg(QString::number(id)); }
...
AlsaDevice::AlsaDevice(AlsaDevice const &ad) : id_(ad.id()), currentMixerId_(ad.currentMixerId_) { }
