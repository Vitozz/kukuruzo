...
SettingsDialog::SettingsDialog(QWidget *parent) :
    QDialog(parent), ui(new Ui::SettingsDialog), soundCards_(QStringList()), mixers_(QStringList()), isAutorun_(false),
    pulseAvailable_(false), playbacks_(new QListWidget(this)), captures_(new QListWidget(this)),
    enums_(new QListWidget(this)), l1_(new QLabel(tr("Playback Switches"), this)),
    l2_(new QLabel(tr("Capture Switches"), this)), l3_(new QLabel(tr("Enum Switches"), this))
{
    ui->setupUi(this);
...
void SettingsDialog::connectSignals()
{
    ...
    connect(ui->cardBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [this](int index) { emit soundCardChanged(index); });
...
void SettingsDialog::setAutorun(bool isAutorun) { ui->isAutorun->setChecked(isAutorun); }
...
void SettingsDialog::onPulseSoundSystem(bool toggled) { emit soundSystemChanged(toggled); }

void SettingsDialog::setUsePulse(bool isPulse) { ui->usePulseaudio->setChecked(isPulse); }

void SettingsDialog::hideAlsaElements(bool isHide) { ui->mixerBox->setEnabled(!isHide); }
...
void SettingsDialog::setUsePolling(bool isPoll) { ui->enableTimer->setChecked(isPoll); }
