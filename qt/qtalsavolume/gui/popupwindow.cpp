...
#endif
    mixerName_(QString()), cardIndex_(0), mixerList_(QStringList()), switchList_(MixerSwitches::Ptr()),
    playBackItems_(QList<switcher>()), captureItems_(QList<switcher>()), enumItems_(QList<switcher>()),
    trayIcon_(new TrayIcon()), mainLayout_(new QVBoxLayout()), volumeSlider_(new QSlider(Qt::Vertical, this)),
    volumeLabel_(new QLabel(this)), pollingTimer_(new QTimer(this)), settingsDialog_(new SettingsDialog()),
    cardName_(QString()), pulseCardName_(QString()), volumeValue_(0), pollingVolume_(0), isMuted_(false),
    isAutorun_(false), isLightStyle_(false), isPulse_(false), isPoll_(true), title_(tr("About QtAlsaVolume")),
                 ...
                 .arg(APP_VERSION))
{
    ...
    if (pulse_->available()) {
        ...
        pulseCardName_
            = (!lastSink.isEmpty() && pulse_->isDeviceAvailable(lastSink)) ? lastSink : pulse_->defaultSink();
...
#endif
    settingsDialog_->setWindowIcon(QIcon(isLightStyle_ ? appLogoLight : appLogoDark));
...
void PopupWindow::setIconToolTip(int value)
{
    ...
    if (isPulse_) {
        if (pulse_) {
            const QString message = tr("Card: %1%2%3")
...
#endif
    if (!isPulse_) {
        const QString message = tr("Card: %1%2%3%4")
...
void PopupWindow::onPlayback(const QString &name, bool isIt) { alsaWork_->setSwitch(name, PLAYBACK, isIt); }

void PopupWindow::onCapture(const QString &name, bool isIt) { alsaWork_->setSwitch(name, CAPTURE, isIt); }

void PopupWindow::onEnum(const QString &name, bool isIt) { alsaWork_->setSwitch(name, ENUM, isIt); }
